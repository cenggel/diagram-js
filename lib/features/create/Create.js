...
export default function Create(
  ...
  function canCreate(elements, target, position, source) {
    ...
    var shape = elements.length === 1 && elements[0];
  ...
  eventBus.on('create.move', function(event) {
    ...
    canExecute = context.canExecute = hover && canCreate(elements.getAll(), hover, position, source);
  ...
  eventBus.on('create.end', function(event) {
    ...
    if (connect) {
      ...
      shape = elements.find(function(element) {
  ...
  this.start = function(event, elements, context) {
    if (elements instanceof Elements === false) {
      elements = new Elements(elements);
    }

    var shape = elements.find(function(element) {
      return !isConnection(element);
    });
